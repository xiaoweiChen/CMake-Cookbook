...
本示例的代码示例是复用第3章第9节的配置，并进行微小的改动。致歉，我们展示了如何在您的系统上找到ZeroMQ库并将其链接到一个C程序中。本示例中，在生成实际的C++程序之前，我们将检查一个使用GNU/Linux上的系统UUID库的小型C程序是否能够实际运行。
...
2. 我们需要在系统上找到UUID库。这通过使用`pkg-config`实现的。我们要求搜索返回一个CMake导入目标使用`IMPORTED_TARGET`参数:
...
3. 接下来，我们需要使用`CheckCSourceRuns.cmake`模块。C++的是`CheckCXXSourceRuns.cmake`模块。但到CMake 3.11为止，Fortran语言还没有这样的模块:
...
5. 我们声明`CMAKE_REQUIRED_LIBRARIES`变量来对`check_c_source_runs`函数的调用。接下来，我们调用`check_c_source_runs`，其中测试代码作为第一个参数，`_runs`变量作为第二个参数，以保存执行的检查结果。我们还取消了`CMAKE_REQUIRED_LIBRARIES`变量的设置:
...
6. 如果检查没有成功，要么是因为代码段没有编译，要么是因为它没有运行，我们会用致命的错误停止配置:
