...
`main.cpp`、`sum_integers.cpp`和`sum_integers.hpp`与之前相同，但将修改`test.cpp`:
...
2. 然后引入一个`if`，检查`ENABLE_UNIT_TESTS`。默认情况下，它为`ON`，但有时需要设置为`OFF`，以免我们没有任何网络下载时，使用Google Test源:
