...
Boost库提供了丰富的C++基础设施，在C++开发人员中很受欢迎。第3章中，已经展示了如何在系统上找到Boost库。然而，有时系统上可能没有项目所需的Boost版本。这个示例将展示如何利用超级构建模式来交付代码，并确保缺少的依赖项不会让CMake停止配置。我们将重用在第3章第8节的示例代码，但是以超构建的形式重新组织。这将是项目的文件结构:
...
您将注意到项目源代码树中有四个CMakeLists.txt文件。下面的部分将详细对这些文件进行了解。

## 具体实施

`  external/upstream/boost `中的CMakeLists.txt描述了满足对boost的依赖所需的操作。我们的目标很简单，如果没有安装所需的版本，下载源存档文件并构建它:
...
3. 如果`find_package`没有成功，或者正在强制进行超级构建，我们需要建立一个本地构建的Boost。为此，我们进入前一个条件句的`else`部分:
...
5. 我们准备了基于所需组件构建的库列表。我们定义了一些列表变量:`_build_byproducts`，包含要构建的库的绝对路径;`_b2_select_libraries`，包含要构建的库的列表;和`_bootstrap_select_libraries`，这是一个字符串，与`_b2_select_libraries`具有相同的内容，但格式不同:
...
6. 现在，我们可以将Boost项目添加为外部项目。首先，我们在下载选项类中指定下载URL和checksum。`DOWNLOAD_NO_PROGRESS`设置为1，以禁止打印下载进度信息:
...
10. 最后，我们将库列表为`BUILD_BYPRODUCTS`并关闭
...
12. 在条件的`else`分支中，执行的最后一个操作是取消所有内部变量的设置:
...
2. 我们调用`find_package`寻找项目依赖的Boost。从主CMakeLists.txt中配置的项目，可以保证始终满足依赖关系，方法是使用预先安装在系统上的Boost，或者使用我们作为子项目构建的Boost:
...
导入目标的使用虽然很简单，但不能保证对任意Boost和CMake版本组合都有效。这是因为CMake的`FindBoost.cmake`模块会创建手工导入的目标。因此，当CMake有未知版本发布时，可能会有`Boost_LIBRARIES`和`Boost_INCLUDE_DIRS`，有导入目标的情况(https://stackoverflow.com/questions/42123509/cmake-finds-boost-but-the-imported-targets-not-available-for-boost-version )。
...
3. `xternal/upstream/boost/CMakeLists.txt `将处理Boost的依赖。
4. 最后，`src`下的`CMakeLists.txt`将构建我们的示例代码(其依赖于Boost)。

从`  external/upstream/boost/CMakeLists.txt `文件开始讨论。Boost使用它自己的构建系统，因此我们需要在`ExternalProject_Add`中配置的详细一点，以便正确设置所有内容:
...
6. 然后，来看安装选项。Boost使用本地构建工具管理安装。事实上，构建和安装命令可以很容易地整合为一个命令。
...
将这些变量设置成新构建的Boost安装路径，我们可以确保正确地获取依赖项。
