...
如果代码是开源的，那么用户将希望能够下载项目的源代码，并使用完全定制的CMake脚本自行构建。当然，打包操作可以使用脚本完成，但是CPack提供了更简单和可移植的替代方案。本示例将指导您创建一些包:

* **源代码包**：可以将源代码直接压缩成归档文件，进行发送。用户将不必担心特定的版本控制。
* **二进制包**：使用这些工具将新构建的目标以打包的方式到归档文件中。它们可能非常有用，但可能不够健壮，无法分发库和可执行程序。
* **平台原生的二进制安装**：CPack能够以许多不同的格式生成二进制安装程序，因此您可以将软件分发到许多不同的平台。我们将展示如何生成安装程序:
...
由于本示例的重点是使用CPack，所以不评论源代码本身。我们只会在`CMakeCPack.cmake`中添加打包指令。此外，我们还添加了`INSTALL.md`和`LICENSE`文件：打包指示要求，需要包含安装说明和项目许可。
...
CPack可用于生成用于分发的包。生成构建系统时，我们在`CMakeCPack.cmake`中列出了CPack指令，用于在构建目录下生成` CPackConfig.cmake`文件。当运行以`package`或`package_source`目标的CMake命令时，CPack会自动调用，参数是自动生成的配置文件。实际上，这两个新目标是对CPack简单规则的调用。与CMake一样，CPack也有生成器的概念。CMake上下文中的生成器是用于生成本地构建脚本的工具，例如Unix Makefile或Visual Studio项目文件，而CPack上下文中的生成器是用于打包的工具。我们列出了这些变量，并对不同的平台进行了特别的关注，分别为源包和二进制包使用了`CPACK_SOURCE_GENERATOR`和`CPACK_GENERATOR`变量。因此，`DEB`包生成器将调用`Debian`打包实用程序，而`TGZ`生成器将调用给定平台上的归档工具。我们可以直接在`build`目录中调用CPack，并选择要与`-G`命令行选项一起使用的生成器。`RPM`s包可以通过以下步骤生成:
...
对于GNU/Linux，节配置了`DEB`和`RPM`生成器:
...
我们的示例依赖于UUID库，`CPACK_DEBIAN_PACKAGE_DEPENDS`和`cpack_rpm_package_require`选项允许指定，包和数据库中对其他包的依赖关系。我们可以使用dpkg和rpm程序分别分析生成的`.deb`和`.rpm`包的内容。

注意，`CPACK_PACKAGING_INSTALL_PREFIX`也会影响这些包生成器：我们的包将安装到`/opt/recipe-01`。

CMake真正提供了跨平台和可移植构建系统的支持。下面的小节将使用Nullsoft脚本安装系统(NSIS)创建一个安装程序:
...
最后，如果在macOS上构建项目，下面的小节将启用`Bundle packager`:
...
在macOS示例中，需要为包配置一个属性列表文件，这是通过`configure_file`实现的。`Info.plist`的位置和包的图标,都可以通过CPack的变量进行设置。

**NOTE**:*可以在这里阅读，关于属性列表格式的更多信息:https://en.wikipedia.org/wiki/Property_list*

## 更多信息

对` CMakeCPack.cmake `进行设置，要比列出CPack的配置选项简单的多，我们可以将`CPACK_*`变量的每个生成器设置放在单独的文件中，比如`CMakeCPackOptions.cmake`，并将这些设置包含到CMakeCPack中。cmake使用`set(CPACK_PROJECT_CONFIG_FILE "${PROJECT_SOURCE_DIR}/CMakeCPackOptions.cmake")`将设置包含入` CMakeCPack.cmake`中。还可以在CMake时配置该文件，然后在CPack时包含该文件，这为配置多格式包生成器提供了一种简洁的方法(参见https://cmake.org/cmake/help/v3.6/module/CPack.html )。
